...
Образ несёт оба языка сразу и выбирает нужный на ходу, поэтому надписи пишутся
через `TR()`:
...
Для таблиц надписей есть `TRA()` — он выбирает один из двух массивов. Если вам
нужен один язык, напишите одну и ту же строку дважды, ничего не сломается.
В `module.json` подпись в меню тоже двуязычная: `name` и `name_ru`.

## Что можно звать

Всё объявлено в заголовках из `dist/sdk/include` (они же `work/src/*.h`):

Полный справочник — [api.md](../api.md) (на английском).
